...
(3 points) The following code roughly reproduces figure 5.5 from the textbook [@book{abdi_experimental_2009}]. Modify the ggplot code so that the graph looks as close as possible to the figure from textbook:
...
`ggplot(slamecka_design,aes(x=number_of_IL,
  ...
  scale_x_continuous(breaks=c(2,4,8), limits=c(0,8))`
```  
